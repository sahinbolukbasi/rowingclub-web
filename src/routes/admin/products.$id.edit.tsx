...
function EditProductPage() {
  ...
  const toggleColor = (n: string) =>
    setSelectedColors((p) => (p.includes(n) ? p.filter((c) => c !== n) : [...p, n]));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const s of selectedSizes) {
      const v = parseInt(stockPerSize[s] || "0", 10);
      if (v > 0) sps[s] = v;
    }
  ...
  return (
    <div className="min-h-screen bg-ink text-paper">
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-3xl px-6 py-8 lg:px-10 space-y-6">
        ...
        <div className={`rounded-xl border p-5 transition ${isClosed ? "border-crim/50 bg-crim/10 shadow-lg shadow-crim/10" : "border-paper/20 bg-ink/40"}`}>
            ...
            <div className="mt-3 flex items-center gap-3 flex-wrap">
              ...
              {discPrice !== null && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-display text-crim font-bold">₺{discPrice}</span>
                </div>
              )}
            </div>
          )}
